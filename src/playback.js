...
module.exports = class Playback {
  ...
  async play(skipped = false) {
    ...
    if (!this.connection) {
      ...
      const voiceChannel = user.lastMessage.member.voiceChannel;
      if (!voiceChannel) {
        song.message.send(
          "your request will not be played because you disconnected from the voice channel!"
        );
    ...
    song
      .play(this.connection.channel.bitrate)
      .then(stream => {
        this.stream = stream;
        ...
        this.dispatcher.on("start", async () => {
          ...
          song.flags.forEach(flag => {
            ...
            if (Number.isInteger(number)) {
              this.volume = (number * (default_volume * 1)) / 100;
        ...
        this.dispatcher.on("end", async reason => {
          ...
          if (
            ...
            if (
              (prev.flags.indexOf("loop") !== -1 && reason === "user") ||
          ...
          this.play(reason === "user").catch(error =>
            song.message.send(
              "an error occurred while attempting to initiate playback: " +
                error
            )
          );
      ...
      .catch(error => {
        ...
        this.playing.message.send(
          "The following error occurred while attempting to play: " +
            "`" +
            this.playing.title +
            "`!\n " +
            "`" +
            error +
            "`" +
            (this.queue.size() > 0 ? ". Playing next item in queue..." : "")
        );
        this.play(true).catch(error =>
          song.message.send(
            "an error occurred while attempting to initiate playback: " + error
          )
        );
      });
  }
  ...
  async replay(message, args) {
    ...
    if (!item) return message.send("I don't remember what played before this!");
    ...
    if (this.playing)
      this.skip().catch(error =>
        message.send("an error occurred while skipping tracks: " + error)
      );
    else
  ...
  addFlag(flag, message, apply_on_queue = false) {
    const item = this.queue.history[this.queue.history.length - 1];
    if (item.flags.indexOf(flag) !== -1)
      return message.send("item already has the `" + flag + "` attribute!");
    if (apply_on_queue) this.queue.queue.forEach(elem => elem.flags.push(flag));
    item.flags.push(flag);
    message.send("added `" + flag + "` functionality to " + "the queue!");
  }

  setVolume(message, val) {
    if (val === this.volume) return;
    if (!val) {
      const vol = Math.round((this.volume / (default_volume * 4)) * 100);
      if (message) return message.send("playing at " + `${vol}% volume!`);
      return vol;
    }
  ...
  queueTime(queue = this.queue.queue) {
    ...
    let containsLivestream =
      queue === this.queue.queue ? this.playing.duration === 0 : false;
    ...
    if (this.dispatcher && queue === this.queue.queue) {
      ...
      if (
        this.playing.duration === 0 ||
        this.playing.flags.indexOf("loop") !== -1
      )
        return "∞";
      total += Math.round(this.playing.duration - time / 1000);
    }
